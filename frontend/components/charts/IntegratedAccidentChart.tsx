...
// 메인 컴포넌트
export default function IntegratedAccidentChart({ data, loading }: IntegratedAccidentChartProps) {
  // 🔧 모든 hooks를 컴포넌트 최상단으로 이동 (순서 일관성 보장)
  ...
  // 3. 차트 데이터 준비 - 연도별 데이터와 사업장별 데이터를 통합
  ...
  // 4. 사업장 목록 추출 (모든 연도 데이터에서 사업장 정보 수집)
  ...
  // 6. 유틸리티 함수들을 useCallback으로 최적화
  const getLineLabelPosition = React.useCallback((index: number, totalPoints: number, value: number, yAxisRange: { min: number, max: number }) => {
  ...
  const getBarLabelPosition = React.useCallback((index: number, totalPoints: number, value: number, yAxisRange: { min: number, max: number }) => {
    const isFirstPoint = index === 0;
    const isLastPoint = index === totalPoints - 1;
    const isHighValue = value > (yAxisRange.max + yAxisRange.min) / 2;
  ...
  // 🔧 조건부 렌더링을 마지막에 처리 (hooks 순서 일관성 보장)
